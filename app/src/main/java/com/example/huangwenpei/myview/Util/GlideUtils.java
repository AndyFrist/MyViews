...
public class GlideUtils {
    ...
    //加载圆角图片
    public static void loadRoundImageView(Context mContext, String path, ImageView mImageView,int radius) {
       Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).transform(new CornersTransform(mContext,radius)).into(mImageView);
    }
    //默认加载
    public static void loadImageCallback(Context mContext, String path, ImageView mImageView,final ImageLoadCallBack callBack) {
    ...
    public static Bitmap getBitMap(String path){
        ...
        try {
             myBitmap =  Glide.with(MyApp.getContext())
                    .load(path)
                    .asBitmap() //必须
                    .centerCrop()
                    .into(500, 500)
